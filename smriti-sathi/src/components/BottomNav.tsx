...
export default function BottomNav({ activeTab, onTabChange, role }: BottomNavProps) {
  ...
  return (
    ...
    >
      <div className="pointer-events-auto flex items-center gap-1 p-1.5 rounded-full border border-[var(--color-border)] bg-[color-mix(in_srgb,var(--color-card)_92%,transparent)] backdrop-blur-xl shadow-[0_12px_32px_rgba(15,23,42,0.16),0_4px_12px_rgba(15,23,42,0.10)] max-w-[min(560px,100%)] w-[min(560px,100%)] justify-between overflow-x-auto scrollbar-none">
        {tabs.map(({ id, label, Icon }) => {
          ...
          return (
            <button
              ...
              className={`relative flex-1 min-w-0 flex flex-col items-center justify-center gap-1 rounded-full px-2.5 py-2.5 transition-all duration-200 ease-[cubic-bezier(0.16,1,0.3,1)] focus:outline-none focus-visible:ring-4 focus-visible:ring-indigo-500/20 ${
                isActive
            ...
            >
              ...
              <span className={`text-[10px] leading-none tracking-wide whitespace-nowrap ${isActive ? 'font-extrabold' : 'font-semibold'}`}>
                {label}
              </span>
