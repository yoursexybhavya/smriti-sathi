...
if [ -f "$APK_SOURCE" ]; then
  ...
  cp "$APK_SOURCE" "SmritiSathi-v2.3.0.apk"
  echo "✓ Created $APK_TARGET and SmritiSathi-v2.3.0.apk ($(du -h "$APK_TARGET" | cut -f1))"
  
  if [ -d "/Users/krishnajangid/Desktop" ]; then
    cp "$APK_SOURCE" "$DESKTOP_TARGET"
    cp "$APK_SOURCE" "/Users/krishnajangid/Desktop/SmritiSathi-v2.3.0.apk"
    echo "✓ Copied fresh APK to $DESKTOP_TARGET and Desktop/SmritiSathi-v2.3.0.apk"
  fi
