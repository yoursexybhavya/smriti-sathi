...
async function generatePresentation() {
  ...
  // =========================================================================
  {
    ...
    // Speaker Notes
    slide.addNotes(
      "Respected judges, imagine standing before the 400-year-old Amer Fort or Taj Mahal. 90% of India's cultural tourists visit the same 15 famous monuments, while over 3,500 national treasures stand in complete digital silence. Today, Team Herodotus is proud to present a working, map-first digital companion that gives living stone a voice in every traveler's pocket, unifying 3,693 ASI monuments into an instant web experience with zero app downloads."
    );
  }
  ...
  // =========================================================================
  {
    ...
    // Speaker Notes
    slide.addNotes(
      "When a traveler arrives at an ASI monument like Hawa Mahal, the digital experience collapses. Official signboards are weathered and unreadable. Commercial audio guides cost ₹300–₹500 and exist only in English at fewer than 30 sites. Wikipedia articles are unformatted walls of text. The physical history is breathtaking, but the contextual narrative is completely absent. 300 million domestic tourists walk past 3,693 protected sites with zero verified digital context."
    );
  }
  ...
  // =========================================================================
  {
    ...
    // Speaker Notes
    slide.addNotes(
      "Instead of forcing users into an outdated keyword text-search box, Herodotus reimagines monument discovery through dynamic spatial hierarchy. We asked a fundamental question: Why are we typing monument names into search boxes when monuments are physical coordinates on earth? As you zoom into India, the map dynamically clusters 3,693 ASI monuments. Tap Amer Fort, and an instant, verified visitor card unfolds with zero latency, complete with an in-browser audio narrative in multiple regional Indian languages."
    );
  }
  ...
  // =========================================================================
  {
    ...
    // Speaker Notes
    slide.addNotes(
      "Here is our working MVP in action. Built as a progressive web application, Herodotus requires no App Store download and consumes less than 350 kilobytes on initial load. Travelers simply open the URL, locate their monument, tap to play client-side audio, and access verified ASI timings and ticket booking links. The 4-step vertical flow takes a visitor from national exploration to mother-tongue narration in under 10 seconds."
    );
  }
  ...
  // =========================================================================
  {
    ...
    slide.addText('Every layer is an existing browser or platform capability. The monument dataset starts as a spreadsheet and exports to JSON — so content can grow without touching the code.', {
    ...
    // Speaker Notes
    slide.addNotes(
      "Our technical feasibility stems from intentional simplicity. By utilizing browser-native Web Speech synthesis and client-side vector clustering on static GeoJSON data hosted on global Edge CDN, we achieve zero marginal server cost per audio listener. There are no expensive backend databases or heavy cloud streaming pipelines to maintain in the MVP. The initial bundle loads in under 350 kilobytes on budget 4G networks, and new monuments are onboarded in under 48 hours."
    );
  }
  ...
  // =========================================================================
  {
    ...
    // Speaker Notes
    slide.addNotes(
      "History is etched into every stone across India. With Herodotus, that history can finally speak. We have a live, working MVP ready for demo right now at herodotus-guide.vercel.app. We invite the judges to scan the QR code and explore India's living heritage on your own smartphones. Thank you, respected judges, and we are now ready for your questions."
    );
  }
...
with open('/Users/krishnajangid/Documents/antigravity/peaceful-hertz/generate_deck.js', 'w') as f:
    f.write(code)

print("Successfully written updated generate_deck.js")
